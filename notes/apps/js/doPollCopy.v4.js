...
dragPane.pollCopy = function() {
	...
	if ( ! this.url  || ! this.putData ) {
        ...
        if ( this.polling == false ) {
            console.log("_GroupDragPane.pollCopy    this.polling: " + this.polling + ". Returning");
            return;
        }
...
dragPane.stopPollCopy = function() {
// STOP POLLING SERVER
	console.log("_GroupDragPane.stopPollCopy    Doing this.polling = false");
	this.polling = false;
//	this.timer.onEndPoll();


};

var doStopPollCopy = function () {
    console.log("_GroupDragPane.stopPollCopy    Doing this.stopPollCopy");

    // SET RESPONSE STATUS
    dragPane.url = Agua.cgiUrl + "/t/test.cgi?response={'status':'completed'}&";
    console.log("_GroupDragPane.stopPollCopy    Setting dragPane.url: " + dragPane.url);

//    dragPane.stopPollCopy();
};
