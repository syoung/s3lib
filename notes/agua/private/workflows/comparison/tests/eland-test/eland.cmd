/nethome/syoung/0.5/bin/apps/logic/loop.pl \
--replicates 1-2 \
--loop parallel \
--executable /nethome/syoung/0.5/bin/apps/aligners/ELAND.pl \
--inputfiles /scratch/syoung/base/pipeline/SRA/NA18507/SRP000239/sampled/200bp/100M/simpleheader/yoruba1-%REPLICATE%.reads_1.10000.sequence.txt \
--matefiles /scratch/syoung/base/pipeline/SRA/NA18507/SRP000239/sampled/200bp/100M/simpleheader/yoruba1-%REPLICATE%.reads_2.10000.sequence.txt \
--referencedir /nethome/bioinfo/data/sequence/chromosomes/human/hg19/eland/chr22 \
--outputdir /scratch/syoung/base/pipeline/SRA/NA18507/SRP000239/sampled/200bp/chr22/eland-test/%REPLICATE% \
--inputtype fastq \
--species human \
--label eland-test-%REPLICATE% \
--maxjobs 2000 \
--walltime 24 \
--queue small \
--cluster LSF \
--cpus 1 \
--reads 1000 \
--stdout /scratch/syoung/base/pipeline/SRA/NA18507/SRP000239/sampled/200bp/chr22/eland-test/%REPLICATE%/eland-test.out \
--loopout /scratch/syoung/base/pipeline/SRA/NA18507/SRP000239/sampled/200bp/chr22/eland-test/loop-eland-test.out