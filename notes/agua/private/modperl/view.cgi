...
print "{ error: 'view.cgi    json not defined' }\n" and exit if not defined $json;

#### GET MODE
my $mode = $json->{mode};
print "{ error: 'view.cgi    mode not defined' }\n" and exit if not defined $mode;
...
#### GET CLASS
my $class = $json->{class} || "View";
#$class = "NotFound";
print "{ error: 'view.cgi    class not defined or empty' }" and exit if not defined $class or not $class;
my $classfile    = FindBin::Real::Bin() . "/lib/Agua/$class.pm";
print "{ error: 'view.cgi    classfile not found: $classfile' }" and exit if not -f $classfile;
eval { require $classfile };
print "{ error: 'view.cgi    require classfile failed: $classfile' }" and exit if $@ and not $INC{"Agua/$class.pm"};
