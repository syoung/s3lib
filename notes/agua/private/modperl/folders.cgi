...
{
	print "{ error: 'folders.cgi: input not defined' }";
...
print "{ error: 'folders.cgi    json not defined' }\n" and exit if not defined $json;

#### GET MODE
my $mode = $json->{mode};
print "{ error: 'folders.cgi    mode not defined' }\n" and exit if not defined $mode;

#### GET USERNAME
my $username = $json->{username};
print "{ error: 'folders.cgi    username not defined' }" and exit if not defined $username;

#### GET CLASS
my $class = $json->{class} || "Folders";
#$class = "NotFound";
print "{ error: 'folders.cgi    class not defined or empty' }" and exit if not defined $class or not $class;
my $classfile    = FindBin::Real::Bin() . "/lib/Agua/$class.pm";
print "{ error: 'folders.cgi    classfile not found: $classfile' }" and exit if not -f $classfile;
eval { require $classfile };
print "{ error: 'folders.cgi    require classfile failed: $classfile' }" and exit if $@ and not $INC{"Agua/$class.pm"};
