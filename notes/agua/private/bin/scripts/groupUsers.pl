...
if ( -d $basedir )
{
    ...
    if ( $overwrite )
    {
        #print "Overwrite directory selected\n";
        ...
        #print "Archives (before sort): *@archives*\n";
        @archives = sort @archives;
        shift @archives; shift @archives;   #### REMOVE '..' AND '.'
        close(DIR);
        #print "Archives (after sort): *@archives*\n";
        ...
        foreach my $directory ( @directories )
        {
            ...
            print ".";
            #print "Moving $fullpath to archive: $archive/$directory\n";
...
foreach my $directory ( @directories )
{
    ...
    #print "Copying $fullpath TO $basedir/$directory\n";
...
#print "install groupUsers: $install_groupUsers\n";
...
#print "Changed in conf file: $default_groupUsers\n";
#print `cat $default_groupUsers`;
...
sub installation_parameters
{
    ...
    if ( $mode =~ /^custom$/ )
    {
        ...
        if ( $input !~ /^\s*$/ )
        {
            #print "Alternate base directory selected: $input\n";
            $basedir = $input;
        }
        ...
        if ( $input !~ /^\s*$/ )
        {
            #print "Alternate web directory selected: $input\n";
            $webdir = $input;
        }
...
sub linkdir
{
    ...
    #print "Removing existing link: $target...\n";
