...
perl saveDb.pl --db agua --outputdir /agua/0.6/bin/sql/tsv/110808
...
print "saveDb.pl    dbtype: $dbtype\n" if $DEBUG;
print "saveDb.pl    user: $user\n" if $DEBUG;
print "saveDb.pl    password: $password\n" if $DEBUG;
print "saveDb.pl    database: $database\n" if $DEBUG;

#### CREATE DB OBJECT USING DBASE FACTORY
my $dbobject;
if ( $dbtype eq "SQLite" )
{
	print "saveDb.pl    Creating SQLite dbobject\n" if $DEBUG;
	my $db = $conf->getKey("database", 'DBFILE');
	print "saveDb.pl    db: $db\n" if $DEBUG;
	$dbobject = Agua::DBaseFactory->new( "SQLite", { 'DBFILE' => "$db" } );
}
elsif ( $dbtype eq "MySQL" )
{
	print "saveDb.pl    Creating MySQL dbobject\n" if $DEBUG;
	#### CREATE DB OBJECT USING DBASE FACTORY
	$dbobject = Agua::DBaseFactory->new( 'MySQL',
		{
			database	=>	$database,
			user      	=>  $user,
			password  	=>  $password
