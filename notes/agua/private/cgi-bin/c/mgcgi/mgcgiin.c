...
void MGCGILoadInput( void )
{
	...
	if( MGCGIInputFile )
	{
		...
		if( (MGCGIInput=calloc( 1, InputLength+2 )) == NULL )
		{
			fclose( Src );
	...
	else
	{
		...
		{	
			...
			if( !QueryString )
				MGCGIInput = calloc( 2, sizeof(char) );
			else
			{
				MGCGIInput= calloc( strlen(QueryString)+2, sizeof(char) );
				if( MGCGIInput )
		...
		else
		{
			...
			if( (MGCGIInput=malloc( InputLength+2 )) == NULL )
