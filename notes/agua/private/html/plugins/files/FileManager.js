...
NB: ASSUMES ALL SOURCES ARE AT LEAST TWO FOLDERS DEEP IN THE FILESYSTEM
	...
	AND _GroupSelectorPane:
	   ...
	   FileSelector (plugins.files.FileDrag)
		   constructor(args)		
...
constructor : function(args) {
	console.log("FileManager.constructor	args:");
...
setNodes : function () {
	...
	dojo.forEach(nodeNames, function(nodeName) {
		...
		//console.log("Project.remove     node: " + node);
...
remove : function (nodeType) {
	console.log("Project.remove     project.Project.remove(nodeType)");
	console.log("Project.remove     nodeType: " + nodeType);

	// REMOVE EXISTING CONTENT
	while ( this.atomic.nodes[nodeType].firstChild )
	{
		console.log("Project.remove     this.atomic.nodes[nodeType]: " + this.atomic.nodes[nodeType]);
		this.atomic.nodes[nodeType].removeChild(this.atomic.nodes[nodeType].firstChild);
	}

	// REMOVE WIDGETS
	dojo.forEach(this.atomic.widgets[nodeType], function(widget){
		console.log("Project.remove     widget: " + widget);
		if ( widget != null )	widget.destroyRecursive();
	});
},
...
disableMenus : function () {
	console.log("Project.disableMenus     plugins.files.FileManager.disableMenus()");
...
enableMenus : function () {
	console.log("Project.enableMenus     plugins.files.FileManager.enableMenus()");
...
roundRobin : function () {
	console.log("Folders.roundRobin    caller: " + this.roundRobin.caller.nom);
	console.log("Folders.roundRobin    this.titlePanes.length: " + this.titlePanes.length);
	if ( ! this.titlePanes || this.titlePanes.length < 1 )	return;
	var array = this.titlePanes.splice(0, 1);
	var titlePane = array[0];
	console.log("Folders.roundRobin    titlePane.name:" + titlePane.name);
	console.dir({titlePane:titlePane});
	var fakeEvent = {
		stopPropagation : function() {
			console.log("Folders.roundRobin    fakeEvent.stopPropagation()");
		}
	}
...
loadProjects : function () {
	//console.log("Project.loadProjects     plugins.folders.Folders.loadProjects()");
	...
	//console.log("Project.loadProjects     AFTER NEW plugins.folders.ProjectFiles()");
	
	return this.projectFiles.titlePanes;
},

loadSharedProjects : function () {
	//console.log("Project.loadSharedProjects     plugins.folders.Folders.loadSharedProjects()");
	...
	//console.log("Project.loadSharedProjects     AFTER NEW plugins.folders.SharedProjectFiles()");
...
// LOAD SOURCE FILE PANES
	//console.log("Project.loadSources     plugins.folders.Folders.loadSources()");
	...
	//console.log("Project.loadSources     AFTER NEW plugins.folders.SourceFiles()");
	return this.sourceFiles.titlePanes;
},

loadSharedSources : function () {
	//console.log("Project.loadSharedSources     plugins.folders.Folders.loadSharedSources()");
	...
	//console.log("Project.loadSharedSources     AFTER NEW plugins.folders.SharedSourceFiles()");	
},

remove : function (nodeName, widget) {
// REMOVE EXISTING TITLE PANES
	//console.log("Project.remove     nodeName: " + nodeName);
	//console.log("Project.remove     widget: " + widget);
