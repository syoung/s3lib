...
$(document).ready(function() {
	...
	if (authAffil.length) {
		var newP = '<p class="affiliation-list-reveal"><a href="#" class="view-more">+</a>Author Affiliations</p>';
		/* add auth affil show/hide p */
		var contribLists = $("div.article div.contributors ol.contributor-list");
		if (contribLists.length) {
			//contribLists.eq(contribLists.length - 1).after(newP);
		...
		//modClass(authAffil,'hideaffil','showaffil');
		$("div.article div.contributors p.affiliation-list-reveal a.view-more").click(
			function(e) {
				...
				if ($(this).text() == '-') {
					/* hide the affil list */
					allViewMores.empty().append('+');
					//$(this).empty().append('+');
					//modClass($("div.article div.contributors ol.affiliation-list"),'hideaffil','showaffil');
					authAffils.each(
						function(i) {
					...
					//$(this).empty().append('-');
					allViewMores.empty().append('-');
					//modClass($("div.article div.contributors ol.affiliation-list"),'showaffil','hideaffil');
					authAffils.each(
						function(i) {
	...
	var navOrigHeight = gColOrigHeights.col2;
	var gNav = gCol2;
	...
	$(window).scroll(function () {
		if (gNavHeight > 0 && !isIE6) {
			...
			var navBottom = (gNavTopOffset + gColOrigHeights.col2) - offsets.y;
...
debugOut("replacing " + cbItem + " with " + newHTML + " hasdata=" + hasData);
...
function addDockedNav() {
	...
	/*var artSupp = $("#article-cb-main .cb-section ol li:has(a[rel='supplemental-data'])").clone();*/
