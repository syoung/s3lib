...
/* handle collapsing content-box areas, with user prefs */
function prepCollapsibles(parentJQueryEl) {
	prepCollapsibles(parentJQueryEl, true);
}
		...
		function(i) {
			...
			if (id != null) {
				...
				if (!isOpen && !(alreadyHasToggle)) {
					toggleCollapse($this, childListType, "h4 a span.view-more", expandStr, contractStr);
				}
...
function toggleCollapse(collapsibleEl, hideTagExpr, modTextTagExpr) {
	toggleCollapse(collapsibleEl, hideTagExpr, modTextTagExpr, '+', '-');
}
...
function getSiteOption(optionName) {
	return getSiteOption(optionName, gSiteOptionUnknown);
}
